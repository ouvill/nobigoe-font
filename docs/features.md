...
CSSでは、たとえば`font-feature-settings: "liga" 0;`で全角幅濁点・半濁点を分離し、`font-feature-settings: "ss01" 1;`で全角感嘆符・疑問符を斜体明朝へ、`font-feature-settings: "ss04" 1;`で`〜`・`～`を1文字1.25周期の波形へ切り替えられます。のびごえこぶり明朝では、`font-feature-settings: "ruby" 1;`で対応字形をルビ専用字形へ切り替えられます。
