...
Novel本番経路の制作正本は、のびごえ共通字形とOpenType機能を適用済みの`NobigoeVariableMarks-VF.otf`です。`nobigoe-build-variable`はこのCFF2可変フォントから`wght` 200・400・900の三次ベジェpathを取り出し、符号化済みひらがな89字とカタカナ109字、追加済みのU+1B132小書きこ・U+1B155小書きコ、対応する縦組字形と既存`ccmp`出力を同じ意味上のUnicode所有者で変形します。3マスターのpath命令と点数が互換であることを確認し、CFF2の正規化されたdesign location上で300・500・600・700を補間して、標準7位置の輪郭を`NobigoeNovelMincho-VF.otf`へ格納します。元のNobigoe CFF2が持つcmap、送り、縦原点、GSUB/GPOSと、Novel対象外のVariationを維持します。独立したかな制作TTFや別の二次ベジェ変換経路は使用しません。

`か`と`が`の二画目終端は、200／400／900マスターでそれぞれ32／36／44 units短くします。`か`を基字とする濁点・半濁点の`ccmp`出力も同じ意味上の所有字形として収集し、基字と同じ短縮を適用します。筆端は三次ベジェpath上の互換な候補を3マスターで照合し、[`terminal_plans/`](../src/nobigoe_font/terminal_plans/)の字別・横縦・ウェイト別深度を中心線に沿う局所変形へ適用します。横組・縦組ともpath命令、点数、輪郭終端の互換性を保ち、中間ウェイトは変形済みマスター間の輪郭補間で得ます。

Novel CFF2から標準7ウェイトを実体化した後にだけ、漢字字面調整、Libertinus Serif欧文の取り込み、命名、任意の欧文再ヒントを固定OTFへ適用します。したがって可変正本はNovelかなと共通CFF2機能の派生を担い、固定版専用処理を含みません。`--kana-style novel`は引き続き固定フォントを個別に開発・比較する経路であり、この可変本番経路や既定のNoto版出力を変更しません。
