...
`か`と`が`の二画目終端は、200／400／900マスターでそれぞれ32／36／44 units短くします。`か`を基字とする濁点・半濁点の`ccmp`出力も同じ意味上の所有字形として収集し、基字と同じ短縮を適用します。固定CFF経路は中心線に沿った局所輪郭変形を使い、可変制作正本は同じ中心線・減衰設計をTrueTypeの既存点移動へ適用します。横組・縦組とも点数・点順・on/off-curveフラグ・輪郭終端を変えず、中間ウェイトはマスター間の輪郭補間で得ます。
