...
`--han-brush-elements`を指定すると、Noto由来の漢字とGSUBから到達する漢字異体字へ毛筆エレメントを追加します。輪郭コマンド列から、縦画・斜画の起筆と終筆、横画末端のウロコを形状・方向・寸法で照合し、一致したエレメントだけを局所変形します。縦画・斜画は中央の直線を維持し、起筆と終筆の範囲だけを三次ベジェ曲線へ置換します。`十`の縦画のように横画との交差で上下へ分断された輪郭は、位置・方向・太さが連続する場合に同じ一本の筆画として扱います。ウロコは末端の既存曲線の制御点だけを軟らかくし、横画本体・起筆のヒゲ・輪郭端点は変更しません。送り幅と左右サイドベアリングも元の値を保持します。固定OpenType/CFFのNoto版専用オプションで、源暎こぶり明朝版には使用できません。
...
`generate-brush-element-patterns.py`は完全フォントを生成せず、起筆、終筆、ウロコ、横画起筆、点、左払い筆端、右払い筆端、はね、折れ、囲み左下、囲み右下、囲み左上、左払い上部の13種についてA/B/C案を拡大SVGで比較します。選択台帳`tools/brush-element-designs/selection.json`では、起筆B・終筆B・ウロコB・横画起筆B・点C・左払い筆端C・右払い筆端C・はねB・折れB・囲み左下B・囲み右下B・囲み左上B・左払い上部Bを確定しています。比較画面は各エレメントのNoto、A、B、C、源流明朝を表示し、全65輪郭を個別の参考SVGへ書き出します。囲み角では左右下隅の9通り、確定下隅B/Bと左上A/B/Cの3通り、Noto・源流明朝の計14字形を口字全体でも比較します。左払い上部では`右`・`左`・`区`・`有`のNotoと源流明朝、計8字形の実輪郭も併記します。A/B/Cは常に`generate-brush-element-patterns.py`内のコード定義から生成し、Notoと源流明朝は入力フォントの実輪郭をそのままSVG化します。保管済みSVGとブラウザへ読み込んだSVGは判断・指示用の参考データであり、フォント輪郭の入力やコード候補の上書きには使いません。複数輪郭を持つ参照字形では対象輪郭だけを分離できるため、点の比較に永字の他画は混入しません。「選択JSONを保存」では現在の台帳を取得できます。別の台帳は`--selection`、参考SVGの出力先は`--export-svg-dir`で指定します。

点と左右の払いは字形全体を置換しません。Noto輪郭の裁ち落としに接する三次ベジェをde Casteljau分割し、分割点より外側だけを丸い筆端へ置き換えるため、元の曲率とそれ以外の輪郭を保持します。確定したC案は、点で元曲線の外側6%、左払い筆端で外側6%、右払い筆端で片側20%・反対側2.2%だけを置換します。横画起筆Bは源流明朝と同じ二曲線構造を使い、左上端をNotoの`y=431`と源流明朝の`y=439`の中間に近い`y=435`、左下端をNotoの`y=398`と源流明朝の`y=384`の中間`y=391`へ置きます。はねBは`hook-A.svg`の流れを整理し、左端から深いえぐりと右の立ち上がりまでを連続する曲線にします。折れBは源流明朝の三角右辺を構成する三次ベジェを5%地点で分割し、右側95%と`x=884`まで伸びる制御点をそのまま保持します。左上の斜画起点と、分割点までの三角上端だけを確定ウロコBに似た二段の丸みに置き換えます。
...
起筆Bと終筆Bは、源流明朝の`丨`（U+4E28）にある節点構成と制御点配置を採用します。汎用処理ではNoto Serif JPの基準縦画幅68ユニットに対する各点の相対量へ正規化し、検出した縦画幅へ等比で写します。短い縦画では左右方向の形を維持したまま軸方向だけを収まる長さへ圧縮します。基準`丨`へ両方を適用した輪郭は、比較画面で確定した起筆B・終筆Bを結合した輪郭と一致させます。

起筆検出では、長い縦辺だけを先に組み合わせません。`病`・`水`・`草`・`読`のように片側が上端付近で分割された場合は、十分長い反対側辺と露出上端に隣接する短辺を組にします。`唐`・`書`のように両側が横画で分割された短い突出部は、下端から左右へ伸びる横画も確認して独立した短画と区別します。`火`のように縦辺が三次ベジェの場合は上端接線を組み合わせ、起筆側だけをde Casteljau分割して元の払い曲線へ戻します。同じ曲線の下端に払い筆端がある場合は両編集を一つの曲線列へ合成し、中央の曲率と払いを保持します。
...
フォントへの適用ではSVG、Unicode、字形名、固定の点番号を参照しません。`brush.py`は全漢字輪郭を走査し、方向、長さ、平行度、隣接セグメント、相対位置から縦画の上部起筆と下部終筆、横画起筆、ウロコ、点・左右払い・はねに共通する短い筆端、折れ、囲み三隅、左払い上部を検出します。縦画の起筆と終筆は、平行な両側辺を結ぶ局所的な上端・下端輪郭から独立して判定します。輪郭の開始位置をまたぐ上端や、横画との接続によって片側辺が分割された縦画では、露出した起筆を持つ側辺の組を優先します。起筆の検出だけを理由に終筆処理を加えず、横画や枝へ接続する側辺の切れ目も縦画端として扱いません。閉輪郭の開始位置をまたぐ点、横画、ウロコは循環セグメントとして検出します。一方、曲線間の短い水平接続は筆端ではないため、払い・はねの丸めから除外します。検出結果からその場で`_CommandEdit`を生成し、同じ輪郭上で候補が重なる場合はウロコの既存曲線を筆端丸めより優先して、重複しない編集だけを適用します。`command_index`は検出結果にだけ保持し、文字別の座標表は持ちません。可変フォントへ拡張する場合は全マスターで同じマッチと編集トポロジーを得られることを確認し、節点を追加する曲線を全マスターで同じ比率に分割して互換性を保つ必要があります。
...
連結したNoto版の長音は、長い矩形となる中間字形の黒みを`2 × ink area / outline perimeter`で比較し、代表的なひらがなの中央値へ揃えます。横組はY方向、縦組はX方向だけをウェイト別に0〜16 units縮め、始端・中間・終端を同じ比率で補正するため接続面に段差は生じません。源暎こぶり明朝版は連結長音が同版の漢字・ひらがなの中央値以下であるため補正しません。
