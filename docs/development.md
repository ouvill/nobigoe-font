...
点と左右の払いは字形全体を置換しません。Noto輪郭の裁ち落としに接する三次ベジェをde Casteljau分割し、分割点より外側だけを丸い筆端へ置き換えるため、元の曲率とそれ以外の輪郭を保持します。確定したC案は、点で元曲線の外側6%、左払い筆端で外側6%、右払い筆端で片側20%・反対側2.2%だけを置換します。横画起筆Bは元横画の太さ`w`を基準とし、張り出し込みの全高を`w√2`、上下頂点の左右差を`w/√2`にします。下側頂点から接点までを斜め端面長の`√2`倍（`w√5`）とし、上側接点も同じ横位置へ揃えます。頂点と接点を結ぶ上下別々の仮想円は接点で横画へ水平に接し、接続距離が長い上側には大きい半径を使います。はねBは`hook-A.svg`の流れを整理し、左端から深いえぐりと右の立ち上がりまでを連続する曲線にします。折れBは源流明朝の三角右辺を構成する三次ベジェを5%地点で分割し、右側95%と`x=884`まで伸びる制御点をそのまま保持します。左上の斜画起点と、分割点までの三角上端だけを確定ウロコBに似た二段の丸みに置き換えます。
...
起筆Bは白銀比によるK5設計です。縦画幅`w`に対し、左上点を左辺から`w(√2−1)/2`張り出させ、左辺への接続距離を基準点から`w√5`とする仮想円弧で約`3w`かけて収束させます。右上点は右辺から`w(√2−1)`張り出し、独立した短い筆置き曲線で右辺へ戻すため、左右の接続位置は揃えません。汎用処理では検出した縦画幅へ等比で写し、短い縦画では左右方向の形を維持したまま軸方向だけを収まる長さへ圧縮します。終筆Bは源流明朝の`丨`（U+4E28）にある節点構成と制御点配置を、Noto Serif JPの基準縦画幅68ユニットに対する相対量へ正規化して使います。
