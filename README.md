...
OpenType機能を指定できない環境では、連続する波線の直前にASCIIチルダ`~`（U+007E）を入力しても同じ1.25周期へ切り替えられます（例：`~〜〜〜`）。先頭の`~`は最初の波線へ吸収されるため表示されず、文字幅も増えません。`~〜`のように波線が1文字だけの場合は、元の単独字形になります。
...
| feature | 用途 |
|---|---|
| `ccmp` | 全角感嘆符・疑問符合字、濁点・半濁点付き仮名、ASCIIチルダによる波線切替の合成 |
...
| `calt` | 連続する長音・ダッシュ・波線の始端／中間／終端置換と、ASCIIチルダで選択した波線系列の伝播 |
