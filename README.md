...
`font-weight`は200から900まで連続して指定できます。OpenTypeの`calt`が既定で有効な環境で連結し、`vert` / `vrt2`に対応する環境では縦組字形へ切り替わります。
