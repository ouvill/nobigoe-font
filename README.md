...
Noto版では、Noto Serif JPに一体字形がある24列は既存輪郭を使用し、残る167列は基字と結合記号の輪郭を一体化します。生成時にはウェイトごとに基字と記号の輪郭交差を検査し、必要な場合だけ上・横・斜めの外向き候補から移動距離が最短の配置を選びます。源暎こぶり明朝版では、同フォントに既存する88列の一体字形を優先し、残る103列には同フォントの既存字形から測定した専用配置を使用します。小書き仮名では記号を縮小して基字へ近づけ、横組・縦組それぞれに専用字形を生成します。
...
通常のかな、数字、約物と、Manga1が規定する半濁点付き14列（`ㇷ゚`、`か゚`、`き゚`、`く゚`、`け゚`、`こ゚`、`カ゚`、`キ゚`、`ク゚`、`ケ゚`、`コ゚`、`セ゚`、`ツ゚`、`ト゚`）は、OpenTypeの `ruby` で源暎こぶり明朝のルビ専用字形へ置換します。Noto版には源暎こぶり明朝の288字形と配置を1000 units/emへ正規化して移植し、通常仮名の輪郭面積を基準にウェイトごとの線幅を補正します。Noto Serif JPの本文字形とは独立した、小サイズでの使用を前提に調整された輪郭です。小書き仮名、小書きコ、`ㇷ゚`などには縦組専用ルビ字形も用意しています。
